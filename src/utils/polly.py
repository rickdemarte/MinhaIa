...
DEFAULT_VOICE_ID = "Camila"  # Voz brasileira feminina
...
def gerar_audio_polly(texto, nome_arquivo="voz.mp3", voice_id=DEFAULT_VOICE_ID, 
    ...
    polly_client = inicializar_cliente_polly()
    ...
    if len(partes) == 1:
        ...
        _gerar_audio_parte_polly(texto_limpo, nome_arquivo, voice_id, engine, 
        ...
        for i, parte in enumerate(partes, 1):
            ...
            if _gerar_audio_parte_polly(parte, str(nome_parte), voice_id, engine, 
                                       language_code, polly_client):
...
def _gerar_audio_parte_polly(texto, nome_arquivo, voice_id, engine, 
                            language_code, polly_client):
    """Função auxiliar para gerar uma parte do áudio usando AWS Polly"""
    try:
        # Prepara o texto em formato SSML
        ssml_text = criar_ssml_texto(texto)
...
def listar_vozes_disponiveis(language_code=None):
